...
Command.InputCommand.prototype.formatResult = function(args){
	if(args.length>0){
		...
		if(typeof args==='string' && args.length>1){
			try{
				return {source: parseInt(args.substr(0,1)), channel: parseInt(args.substr(1,1)), code: args};
			}catch(e){}
		}
	}
}
...
Command.InputsCommand.prototype.formatResult = function(args){
	...
	for(var i=0; i<args.length; i++){
		...
		if(typeof input==='string' && input.length>1){
			try{
				inputs.push(
					{source: parseInt(input.substr(0,1)), channel: parseInt(input.substr(1,1)), code: input}
				);
			}catch(e){}
		}
	}
